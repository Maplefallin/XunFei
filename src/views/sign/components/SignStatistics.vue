<template>
    <a-row gutter={16} class="row-item" style="margin: 20px;">
      ...
      <a-col span={8} class="col-item">
        <a-card class = 'card-item'>
          <template #title>
            <a-icon type="close-circle" /> 未签到次数
          </template>
  ...
  import type {signinItem} from '@/views/sign/SignInterface'
  ...
  // 计算未签到次数
  const missedCheckIns =props.data.filter(item => item.tags === 'lose').length;
